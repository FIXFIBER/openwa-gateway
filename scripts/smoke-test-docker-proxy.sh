#!/bin/sh
# Smoke test: verify idawhats-api can list containers via docker-socket-proxy.
...
echo "==> Checking idawhats-api health..."
...
PROXY_STATE=$(docker inspect --format='{{.State.Status}}' idawhats-docker-proxy 2>/dev/null || echo "not_found")
if [ "$PROXY_STATE" != "running" ]; then
  echo "FAIL: idawhats-docker-proxy is not running (state: $PROXY_STATE)" >&2
  exit 1
fi
echo "PASS: idawhats-docker-proxy is running"

echo ""
echo "==> Verifying idawhats-api socket mount is gone..."
SOCKET_MOUNT=$(docker inspect idawhats-api --format='{{range .Mounts}}{{.Source}}{{"\n"}}{{end}}' 2>/dev/null | grep "docker.sock" || true)
if [ -n "$SOCKET_MOUNT" ]; then
  echo "FAIL: idawhats-api still has a docker.sock mount: $SOCKET_MOUNT" >&2
  exit 1
fi
echo "PASS: idawhats-api has no direct docker.sock mount"
