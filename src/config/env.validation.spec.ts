...
describe('validateEnv', () => {
  ...
  it('validates POSTGRES_SCHEMA as a legal, non-reserved Postgres identifier when set', () => {
    ...
    expect(() => validateEnv({ ...pg, POSTGRES_SCHEMA: 'idawhats' })).not.toThrow();
    ...
    expect(() => validateEnv({ ...pg, POSTGRES_SCHEMA: 'idawhats; DROP' })).toThrow(/POSTGRES_SCHEMA/);
  ...
  it('rejects a sqlite data DB path that collides with the internal main database file', () => {
    ...
    expect(() => validateEnv({ DATABASE_TYPE: 'sqlite', DATABASE_NAME: './data/idawhats.sqlite' })).not.toThrow();
  ...
  it('rejects a bare SQLite DATABASE_NAME (PG-name leak) that has no path separator or file extension', () => {
    // Regression for #677: .env.example shipped `DATABASE_NAME=idawhats` (a PostgreSQL db name).
    // In a SQLite run that bare name becomes the file PATH → SQLite opens a file named 'idawhats'
    // under the read-only app rootfs → SQLITE_CANTOPEN boot-loop. The guard catches the leak at boot.
    expect(() => validateEnv({ DATABASE_TYPE: 'sqlite', DATABASE_NAME: 'idawhats' })).toThrow(/file path/);
    ...
    expect(() => validateEnv({ DATABASE_TYPE: 'sqlite', DATABASE_NAME: 'idawhats.sqlite' })).not.toThrow();
    ...
    expect(() => validateEnv({ DATABASE_TYPE: 'sqlite', DATABASE_NAME: '/app/data/idawhats.sqlite' })).not.toThrow();
    expect(() => validateEnv({ DATABASE_TYPE: 'sqlite', DATABASE_NAME: './data/idawhats.sqlite' })).not.toThrow();
