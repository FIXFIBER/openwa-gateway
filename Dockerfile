...
RUN if [ "$TARGETARCH" = arm64 ]; then \
        ...
        chown -R idawhats:idawhats /opt/puppeteer && \
...
RUN mkdir -p ./data/sessions ./data/media && \
    chown -R idawhats:idawhats /app

# The non-root idawhats user has no home of its own (`useradd -r`, no -m). Chromium resolves the home
# dir from the passwd entry via glib's getpwuid() — it IGNORES $HOME — so it tries to read/write
# /home/idawhats, which does not exist. On hardened/read-only hosts that makes the browser HARD-CRASH
...
# bypassing the passwd lookup); docker-entrypoint.sh creates them owned by idawhats. On a read_only
# rootfs these live on the tmpfs /tmp. HOME is kept for any other HOME-relative tooling. See #254/#242.
ENV HOME=/app/data
ENV XDG_CONFIG_HOME=/tmp/.config
ENV XDG_CACHE_HOME=/tmp/.cache

# Copy entrypoint: runs as root to fix named-volume ownership, then drops to idawhats via gosu
...
# then drops to the idawhats user via gosu before starting the node process.
