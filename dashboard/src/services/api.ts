// API Service Layer for IdaWhats Dashboard
...
export interface InfraStatus {
  // `builtIn` = IdaWhats's own bundled container is actually running and backing this service (live),
...
async function request<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
  ...
  if (response.status === 401) {
    ...
    sessionStorage.removeItem('idawhats_api_key');
...
export interface CatalogPlugin {
  ...
  license?: string;
  keywords?: string[];
  minIdaWhatsVersion?: string;
  testedIdaWhatsVersion?: string;
  homepage?: string;
  download?: string;
  installed: boolean;
