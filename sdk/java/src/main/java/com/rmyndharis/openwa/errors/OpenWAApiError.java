 ...
 * subclass.
 */
public class IdaWhatsApiError extends IdaWhatsError {
    ...
    public IdaWhatsApiError(String message, int status, Object body, String errorKind) {
        super(message);
        this.status = status;
        this.body = body;
     ...
     * surfaces as a generic {@link IdaWhatsApiError} whose message states the
     * redirect was not followed (the API key is never re-sent to a redirect
     * target).
     */
    public static IdaWhatsApiError fromResponse(int status, String statusText, String rawBody, String context) {
        ...
        String message = "IdaWhats API " + status + reason + " — " + context + tail;
    ...
    public static IdaWhatsApiError classify(int status, String message, Object body, String errorKind) {
        return switch (status) {
            case 401 -> new IdaWhatsAuthError(message, status, body, errorKind);
            case 403 -> new IdaWhatsForbiddenError(message, status, body, errorKind);
            case 404 -> new IdaWhatsNotFoundError(message, status, body, errorKind);
            case 409 -> new IdaWhatsConflictError(message, status, body, errorKind);
            case 429 -> new IdaWhatsRateLimitError(message, status, body, errorKind);
            case 501 -> new IdaWhatsNotImplementedError(message, status, body, errorKind);
            default -> new IdaWhatsApiError(message, status, body, errorKind);
        };
    }
}
