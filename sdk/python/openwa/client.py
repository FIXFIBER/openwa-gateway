"""IdaWhats Python SDK — client core.

The :class:`IdaWhatsClient` is the single entry point. It owns an
...
def _warn_if_insecure_http(url: str) -> None:
    ...
    try:
        ...
        if parsed.scheme == "http" and host not in _LOCALHOST_HOSTS:
            warnings.warn(
                f"IdaWhatsClient: base_url uses an insecure http:// URL (host: {host}). "
...
class IdaWhatsClient:
    def __init__(
        ...
        *,
        ...
        if not base_url:
            raise ValueError("IdaWhatsClient: base_url is required")
        if not api_key:
            raise ValueError("IdaWhatsClient: api_key is required")
    ...
    def __enter__(self) -> "IdaWhatsClient":
        return self
