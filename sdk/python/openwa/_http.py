"""HTTP transport wrapper for the IdaWhats Python SDK.
...
from .errors import IdaWhatsApiError, IdaWhatsTimeoutError, classify
...
class HttpExecutor:
    ...
    Constructed once per :class:`IdaWhatsClient`; the transport is taken from
    ...
    def request(self, method: HttpMethod, path: str, *, query: Mapping[str, Any] | None = None, body: Any = None) -> Any:
        ...
        try:
            ...
            raise IdaWhatsTimeoutError(self._timeout) from e
        ...
        if res.status_code >= 300:
            ...
            raise IdaWhatsApiError.from_response(res.status_code, res.text, context)
