...
import { IdaWhatsClient } from '../src';
import { MockTransport } from './helpers';

function client(t: MockTransport): IdaWhatsClient {
  return new IdaWhatsClient({ baseUrl: 'http://x', apiKey: 'k', fetch: t.asFetch() });
}
...
describe('MessagesResource — exact paths', () => {
  ...
  it('sendVideo / sendAudio / sendDocument / sendSticker use correct segments', async () => {
    const cases: Array<[string, (c: IdaWhatsClient) => Promise<unknown>]> = [
