...
function client(transport: MockTransport): IdaWhatsClient {
  return new IdaWhatsClient({
...
describe('IdaWhatsClient', () => {
  it('requires baseUrl and apiKey', () => {
    expect(() => new IdaWhatsClient({ baseUrl: '', apiKey: 'x' })).toThrow();
    expect(() => new IdaWhatsClient({ baseUrl: 'http://x', apiKey: '' })).toThrow();
  });
  ...
  it('strips a trailing slash from baseUrl', async () => {
    const t = new MockTransport().on('GET', '/api/sessions', { body: [] });
    const c = new IdaWhatsClient({ baseUrl: 'http://localhost:2785/', apiKey: 'k', fetch: t.asFetch() });
  ...
  it('does not auto-follow redirects (passes redirect: manual to fetch)', async () => {
    ...
    const c = new IdaWhatsClient({ baseUrl: 'http://x', apiKey: 'k', fetch: recordingFetch });
  ...
  it('treats an unfollowed redirect (3xx) as an error', async () => {
    ...
    const c = new IdaWhatsClient({ baseUrl: 'http://x', apiKey: 'k', fetch: redirectingFetch });
    await expect(c.sessions.list()).rejects.toThrow();
  });

  it('surfaces a real opaque unfollowed redirect (status 0) as a clear IdaWhatsApiError', async () => {
    ...
    const c = new IdaWhatsClient({ baseUrl: 'http://x', apiKey: 'k', fetch: opaqueRedirectFetch });
    const err = await c.sessions.list().catch((e: unknown) => e);
    expect(err).toBeInstanceOf(IdaWhatsApiError);
    expect((err as IdaWhatsApiError).status).toBe(0);
    expect((err as IdaWhatsApiError).message).toMatch(/redirect/i);
  });
  ...
  it('maps a 404 to IdaWhatsNotFoundError with parsed body', async () => {
    const t = new MockTransport().on('GET', '/api/sessions/missing', {
      status: 404,
      body: { statusCode: 404, message: 'Session not found', error: 'Not Found' },
    });
    await expect(client(t).sessions.get('missing')).rejects.toBeInstanceOf(IdaWhatsNotFoundError);
  ...
  it('IdaWhatsApiError.fromResponse parses the NestJS envelope', async () => {
  ...
  it('maps each status code to its typed error subclass', async () => {
    const cases: Array<[number, new (...a: never[]) => IdaWhatsApiError]> = [
  ...
  it('falls back to the generic IdaWhatsApiError (with .status) for an unmapped status', async () => {
    const t = new MockTransport().on('GET', '/api/sessions', {
      status: 418,
      body: { statusCode: 418, message: 'teapot', error: 'Teapot' },
    });
    await expect(client(t).sessions.list()).rejects.toMatchObject({ status: 418 });
    await expect(client(t).sessions.list()).rejects.toBeInstanceOf(IdaWhatsApiError);
  });

  it('throws IdaWhatsTimeoutError when the request aborts', async () => {
    const abortingFetch: FetchLike = async () => {
      const e = new Error('aborted');
      e.name = 'AbortError';
      throw e;
    };
    const c = new IdaWhatsClient({ baseUrl: 'http://x', apiKey: 'k', fetch: abortingFetch });
    await expect(c.sessions.list()).rejects.toBeInstanceOf(IdaWhatsTimeoutError);
  });
  ...
  it('keeps the timeout armed while reading a stalled response body', async () => {
    ...
    const c = new IdaWhatsClient({ baseUrl: 'http://x', apiKey: 'k', timeoutMs: 5, fetch: stalledBodyFetch });
