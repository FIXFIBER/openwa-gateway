...
import { classifyApiError, IdaWhatsApiError, IdaWhatsTimeoutError } from './errors.js';
...
/**
 * Perform a single request against the IdaWhats API and return the parsed JSON
 * body (or `null` for 204). Throws a typed {@link IdaWhatsApiError} subclass on
 * non-2xx, or {@link IdaWhatsTimeoutError} on timeout.
 */
export async function request<T>(
  ...
  try {
    ...
    if (!res.ok) {
      ...
      const apiError = await IdaWhatsApiError.fromResponse(res, context);
    ...
    if (err instanceof Error && err.name === 'AbortError') {
      throw new IdaWhatsTimeoutError(timeoutMs);
    }
    throw err;
  } finally {
    clearTimeout(timer);
...
export function warnIfInsecureHttpUrl(url: string, label = 'baseUrl'): void {
  try {
    ...
    if (parsed.protocol === 'http:' && !LOCALHOST_HOSTS.has(parsed.hostname.toLowerCase())) {
      console.warn(
        `[IdaWhats SDK] ${label} uses an insecure http:// URL (host: ${parsed.hostname}). ` +
