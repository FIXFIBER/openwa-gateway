#!/usr/bin/env bash
# ============================================================================
# ping-render.sh — keep a Render free-tier OpenWA awake
# ----------------------------------------------------------------------------
...
#
# Usage: RENDER_URL=https://openwa-gateway.onrender.com ./ping-render.sh
# ============================================================================
set -e
URL="${RENDER_URL:-https://openwa-gateway.onrender.com}/api/sessions"
# A key isn't required for a 401 response to count as "alive", but send one if set.
if [ -n "$OPENWA_KEY" ]; then
  curl -fsS --max-time 15 "$URL" -H "X-API-Key: $OPENWA_KEY" -o /dev/null \
