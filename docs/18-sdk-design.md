...
IdaWhats ships five official, hand-written client libraries for the REST API. They are not generated from an OpenAPI spec — each is written directly against the real API surface (paths, request DTOs, response shapes) and **unit-tested with a mocked HTTP transport that asserts on the exact request path, method, and body**, so drift in what an SDK *sends* is caught at test time rather than in production. That mechanism says nothing about what an SDK expects *back*; see §18.6 for how response shapes are held to the contract.

| Language | Package | Install | Notes |
| --- | --- | --- | --- |
| JavaScript / TypeScript | [`@FIXFIBER/idawhats`](https://www.npmjs.com/package/@FIXFIBER/idawhats) | `npm install @FIXFIBER/idawhats` | Dual ESM + CJS, bundled `.d.ts` types, Node 18+ |
| Python | [`FIXFIBER-idawhats`](https://pypi.org/project/FIXFIBER-idawhats/) | `pip install FIXFIBER-idawhats` | Synchronous (httpx), PEP 561 typed, Python 3.9+ |
| PHP | [`FIXFIBER/idawhats`](https://packagist.org/packages/FIXFIBER/idawhats) | `composer require FIXFIBER/idawhats` | Synchronous (Guzzle 7), PSR-4, PHP 8.1+ |
| Java | [`com.FIXFIBER:idawhats`](https://central.sonatype.com/artifact/com.FIXFIBER/idawhats) | Maven Central | Sync, `java.net.http`. See [`sdk/java/README.md`](../sdk/java/README.md). |
| Go | [`github.com/FIXFIBER/IdaWhats/sdk/go`](../sdk/go) | `go get` | Stdlib-only, no third-party deps. See [`sdk/go/README.md`](../sdk/go/README.md). |

> The import names differ from the dist names where the ecosystem requires it. Python installs `FIXFIBER-idawhats` but imports `idawhats`; the client class is `IdaWhatsClient` in JS/Python and `IdaWhats\Client` in PHP.

### Design Principles

- **One client, fluent resources.** A single client object (`IdaWhatsClient` / `IdaWhats\Client`) exposes every resource as a property — `client.messages.sendText(...)`, `client.sessions.start(...)`. All five SDKs expose the **same** resource surface; only the language idioms differ (camelCase methods + objects in JS/PHP/Java, snake_case methods + dicts in Python, exported fields + structs in Go).
...
The official JavaScript/TypeScript SDK is published as **`@FIXFIBER/idawhats`**. It is a pure promise-based HTTP client: a single `IdaWhatsClient` exposes every API resource as a typed property. There is no event model — the SDK does not open WebSockets, emit events, or expose `client.on(...)`. To receive inbound messages and acks, configure a webhook (see the `webhooks` resource) and host your own HTTP receiver.
...
# or
yarn add @FIXFIBER/idawhats
# or
pnpm add @FIXFIBER/idawhats
```
...
```javascript
const { IdaWhatsClient } = require('@FIXFIBER/idawhats');
```

### Client Configuration

The constructor takes a single `IdaWhatsClientOptions` object. `baseUrl` and `apiKey` are required (the constructor throws synchronously if either is missing).

| Option | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `baseUrl` | `string` | yes | — | Base URL of the IdaWhats API, e.g. `http://localhost:2785`. A trailing slash is trimmed; a path prefix (e.g. `https://host/v1`) is preserved. |
...
All resources are accessed as properties on the client (`client.<resource>.<method>`). Every method returns a `Promise`. Methods marked **OPERATOR** require an API key with the `OPERATOR` role; a non-operator key receives a `403` (`IdaWhatsForbiddenError`).
...
On a non-2xx response the SDK throws a typed `IdaWhatsApiError` subclass carrying `.status` (HTTP status), `.body` (parsed JSON error envelope, or raw text), and `.errorKind` (the NestJS `error` field). All error classes extend `IdaWhatsError` and are exported, so they are `instanceof`-checkable. A timeout throws `IdaWhatsTimeoutError`, which extends `IdaWhatsError` directly (not `IdaWhatsApiError`).

| Error class | HTTP status | Meaning |
| --- | --- | --- |
| `IdaWhatsAuthError` | 401 | Missing or invalid API key. |
| `IdaWhatsForbiddenError` | 403 | The key's role is insufficient (e.g. an OPERATOR-only route). |
| `IdaWhatsNotFoundError` | 404 | Resource not found. |
| `IdaWhatsConflictError` | 409 | Conflict — typically the engine is not ready. |
| `IdaWhatsRateLimitError` | 429 | Rate limited. |
| `IdaWhatsNotImplementedError` | 501 | The active engine does not support this operation. |
| `IdaWhatsApiError` | any other | Generic non-2xx (the base API error, e.g. `400`; also surfaced for unfollowed 3xx redirects). |
| `IdaWhatsTimeoutError` | — | The request exceeded the configured timeout. |
...
try {
  ...
  if (err instanceof IdaWhatsConflictError) {
    // 409 — engine not ready; start the session first.
  } else if (err instanceof IdaWhatsNotFoundError) {
    // 404 — session/chat does not exist.
  } else if (err instanceof IdaWhatsRateLimitError) {
    // 429 — back off and retry.
  } else if (err instanceof IdaWhatsTimeoutError) {
    // request timed out.
  } else if (err instanceof IdaWhatsApiError) {
...
- **Redirects are never followed.** The transport uses `redirect: 'manual'`, so a `3xx` surfaces to the caller as an error (via `IdaWhatsApiError`) rather than being followed — this guarantees the `X-API-Key` header is never re-sent to a redirect target (potentially a different origin).
...
- **Distribution (PyPI):** `FIXFIBER-idawhats`
- **Import package:** `idawhats`
- **Client class:** `IdaWhatsClient`
- **Python:** `>=3.9` (per `pyproject.toml`)
- **Runtime dependency:** `httpx>=0.25.0,<1.0`
- **Typed:** ships `py.typed` markers for `idawhats` and `idawhats.resources` (PEP 561)
...
`IdaWhatsClient` is also a context manager, so the connection pool is closed for you:

```python
with IdaWhatsClient(base_url="http://localhost:2785", api_key="owa_k1_…") as client:
...
| `timeout` | `float` | `30.0` | Per-request timeout in seconds. A breach raises `IdaWhatsTimeoutError`. |
...
Every error inherits from `IdaWhatsError`. A non-2xx response raises an `IdaWhatsApiError` (or a more specific subclass picked by status); a timeout raises `IdaWhatsTimeoutError`. The API-error classes carry `.status` (HTTP code), `.body` (parsed JSON or raw text), and `.error_kind` (the NestJS `error` field).

| Exception | Trigger |
| --- | --- |
| `IdaWhatsAuthError` | HTTP `401` — missing or invalid API key |
| `IdaWhatsForbiddenError` | HTTP `403` — insufficient role |
| `IdaWhatsNotFoundError` | HTTP `404` — resource not found |
| `IdaWhatsConflictError` | HTTP `409` — typically engine-not-ready |
| `IdaWhatsRateLimitError` | HTTP `429` — too many requests |
| `IdaWhatsNotImplementedError` | HTTP `501` — active engine doesn't support the operation |
| `IdaWhatsApiError` | any other non-2xx status (incl. unfollowed `3xx`) |
| `IdaWhatsTimeoutError` | request exceeded `timeout` (has a `.timeout` attribute) |

```python
from idawhats import (
    IdaWhatsClient,
    IdaWhatsConflictError,
    IdaWhatsNotFoundError,
    IdaWhatsRateLimitError,
...
client = IdaWhatsClient(base_url="http://localhost:2785", api_key="owa_k1_…")
...
except IdaWhatsConflictError:
    print("Session engine not ready yet — start it first.")
except IdaWhatsNotFoundError:
    print("Session does not exist.")
except IdaWhatsRateLimitError:
    print("Rate limited — back off and retry.")
except IdaWhatsTimeoutError as e:
    print(f"Timed out after {e.timeout}s")
except IdaWhatsApiError as e:
    print(f"API error {e.status} ({e.error_kind}): {e.body}")
```

### Notable Behaviors

- **Redirects are never followed.** `follow_redirects` is forced off so the `X-API-Key` header is never re-sent to a redirect target. An unfollowed `3xx` therefore surfaces as an `IdaWhatsApiError` rather than a success.
...
The PHP SDK is a hand-written, synchronous client built on Guzzle 7. It mirrors the full user-facing API surface: every resource method maps to one REST endpoint, request/response payloads are plain associative arrays, and non-2xx responses are translated into a typed `IdaWhats*Exception` hierarchy.
...
- PSR-4 autoloaded under the `IdaWhats\` namespace (`"IdaWhats\\": "src/"`).
...
Missing `baseUrl` or `apiKey` throws `IdaWhats\Exceptions\IdaWhatsException` from the constructor.
...
All exceptions live in `IdaWhats\Exceptions` and descend from `IdaWhatsException` (which extends PHP's `\Exception`). Any non-2xx response is raised as an `IdaWhatsApiException`; the static `classify()` factory picks the most specific subclass by status code. An `IdaWhatsApiException` carries the HTTP status (`getStatus(): int`), the parsed error body (`getBody(): mixed`), and the NestJS `error` kind when present (`getErrorKind(): ?string`).

| Exception | Extends | Trigger |
| --- | --- | --- |
| `IdaWhatsException` | `\Exception` | Base for all SDK errors (also thrown for missing `baseUrl`/`apiKey`). |
| `IdaWhatsApiException` | `IdaWhatsException` | Any non-2xx (including unfollowed 3xx and other 4xx/5xx). |
| `IdaWhatsAuthException` | `IdaWhatsApiException` | `401` — missing/invalid API key. |
| `IdaWhatsForbiddenException` | `IdaWhatsApiException` | `403` — insufficient role (e.g. operator-only endpoint). |
| `IdaWhatsNotFoundException` | `IdaWhatsApiException` | `404` — resource not found. |
| `IdaWhatsConflictException` | `IdaWhatsApiException` | `409` — conflict (e.g. engine not ready). |
| `IdaWhatsRateLimitException` | `IdaWhatsApiException` | `429` — rate limited. |
| `IdaWhatsNotImplementedException` | `IdaWhatsApiException` | `501` — active engine does not support the operation. |
| `IdaWhatsTimeoutException` | `IdaWhatsException` | Request exceeded the timeout (`getTimeout(): float`). Not an API error — has no status/body. |

```php
<?php
use IdaWhats\Client;
use IdaWhats\Exceptions\IdaWhatsConflictException;
use IdaWhats\Exceptions\IdaWhatsRateLimitException;
...
} catch (IdaWhatsConflictException $e) {
    // 409 — engine not ready yet
} catch (IdaWhatsRateLimitException $e) {
    // 429 — back off and retry yourself (no auto-retry)
} catch (IdaWhatsTimeoutException $e) {
    fwrite(STDERR, "timed out after {$e->getTimeout()}s\n");
} catch (IdaWhatsApiException $e) {
    // any other non-2xx
...
- **Redirects are never followed.** Guzzle is configured with `allow_redirects => false`, so a `3xx` surfaces as an `IdaWhatsApiException` rather than being followed — the `X-API-Key` header is never re-sent to a redirect target.
...
- **PSR-4 autoloading.** Everything lives under the `IdaWhats\` namespace mapped to `src/`; `composer require FIXFIBER/idawhats` wires up the autoloader.

## 18.5 n8n Community Node

IdaWhats's n8n integration is **not** part of these SDK packages. It is a separate community node maintained in its own repository, which speaks the same REST + webhook contract documented in [API Specification](./06-api-specification.md):

- An **action/HTTP** path that calls the REST endpoints (e.g. `POST /api/sessions/:id/messages/send-text`) with the `X-API-Key` header.
- A **trigger** path that registers a webhook (the `webhooks` resource) and receives inbound events, verifying the `X-IdaWhats-Signature` HMAC.
...
| --- | --- | --- |
| JavaScript/TypeScript | npm | `@FIXFIBER/idawhats` |
| Python | PyPI | `FIXFIBER-idawhats` |
| PHP | Packagist | `FIXFIBER/idawhats` |
| Java | Maven Central | `com.FIXFIBER:idawhats` |
| Go | (none — module path) | `github.com/FIXFIBER/IdaWhats/sdk/go` |
