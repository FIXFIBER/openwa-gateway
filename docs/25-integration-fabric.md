...
The overriding goal is to preserve the untrusted-worker safety invariants *by construction*. IdaWhats
...
**~90% a faithful clone of seams IdaWhats already ships**:
...
flowchart LR
    ...
    subgraph Core["IdaWhats host (core)"]
...
- **Authentication inversion.** A provider webhook cannot carry an IdaWhats API key, so ingress is public to
