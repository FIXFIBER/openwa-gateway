...
**Cause:** In production IdaWhats sends the CSP `upgrade-insecure-requests` directive, which tells the
...
docker compose exec idawhats printenv NODE_ENV CSP_UPGRADE_INSECURE_REQUESTS
```
...
> The alternative is to front IdaWhats with a TLS-terminating reverse proxy (the shipped
...
docker compose logs idawhats 2>&1 | grep -i "whatsapp\|puppeteer\|browser"
...
**Fix:** IdaWhats reconciles a missed `ready` event when WhatsApp Web is connected, the injected
...
`$HOME`**. The non-root `idawhats` user has no home dir, so Chromium tries to use `/home/idawhats`, which does
...
owned by `idawhats`). If you run a custom container, ensure both are set to a writable, existing path:
...
```bash
docker stats idawhats-api   # watch the PIDS column — does it climb toward the limit right before the failure?
```

*Fix:* raise the ceiling. The bundled `docker-compose.yml` exposes it as `IDAWHATS_PIDS_LIMIT` (default `2048`,
which fits ~8-10 sessions with startup-spike headroom):

```bash
IDAWHATS_PIDS_LIMIT=4096   # in your .env, then docker compose up -d
```
...
# Docker Desktop: check the VM via the app, or nudge IDAWHATS_MEM_LIMIT up and retry
```

*Fix:* raise the ceiling (`IDAWHATS_MEM_LIMIT=4g` in your `.env`, or Docker Desktop → Settings → Resources →
Memory for the VM).
...
**Quick triage:** run `docker stats idawhats-api`, click **Start**, and watch which resource spikes toward its
...
**Start** after upgrading IdaWhats — no QR is produced — and the session's `lastError` / container log
show:
...
```bash
docker exec idawhats-api rm -rf /app/data/sessions/session-<name>
...
docker compose logs idawhats 2>&1 | grep -i webhook
...
```bash
# Check memory usage
docker stats idawhats --no-stream

# Check process memory (Prometheus text; read idawhats_process_resident_memory_bytes)
...
sqlite3 ./data/idawhats.db ".timeout 30000"

# Increase timeout in configuration
DATABASE_SQLITE_BUSY_TIMEOUT=30000

# Check WAL mode
sqlite3 ./data/idawhats.db "PRAGMA journal_mode;"
# Should return: wal

# Enable WAL mode
sqlite3 ./data/idawhats.db "PRAGMA journal_mode=WAL;"
```
...
docker exec idawhats ping postgres
docker exec idawhats curl http://host.docker.internal:8080
```
...
**Q: Is IdaWhats safe to use?**
> A: IdaWhats uses unofficial WhatsApp Web API. While we implement best practices to avoid detection, there's inherent risk of account restrictions. We recommend:
...
> A: Yes, IdaWhats works with both personal and WhatsApp Business accounts. Note that WhatsApp Business API (official Meta API) is different and not supported.
...
> 2. Set URL: `http://idawhats:2785/api/sessions/{id}/messages/send-text`
...
Traefik forwards WebSocket upgrades automatically, so IdaWhats's single-port Socket.IO channel works with a normal HTTP router. Two things keep a public deployment stable:

**1. Let Traefik reach the container over the Docker network — don't _also_ publish the host port.** This is the most common cause of intermittent `504`s on Coolify/Traefik. If IdaWhats publishes its port to the host (`ports: ["2785:2785"]`) **and** Traefik also routes to it, every request additionally traverses Docker's userland `docker-proxy`. IdaWhats holds a long-lived Socket.IO connection per client (HTTP long-poll → WebSocket upgrade), so those held-open connections accumulate across both hops and gradually exhaust the connection pool to the single upstream — the Dashboard, API, and real-time channel then `504` together "after some time", while `curl http://localhost:2785/api/health/ready` keeps returning `200`. Front it with Traefik on a shared network and **expose** the port internally instead of **publishing** it:
...
services:
  idawhats:
    ...
    labels:
      - traefik.enable=true
      - traefik.http.routers.idawhats.rule=Host(`api.example.com`)
      - traefik.http.routers.idawhats.entrypoints=websecure
      - traefik.http.routers.idawhats.tls.certresolver=le
      - traefik.http.services.idawhats.loadbalancer.server.port=2785
networks:
  proxy:
...
Remember IdaWhats is **single-port**: the Dashboard, REST API, and Socket.IO all share `:2785` behind one router, so a choked upstream takes all three down at once. A Dashboard stuck on "Connecting…" while `localhost` is healthy is the proxy hop, not the app.

**Q: How to backup sessions automatically?**
```bash
# Add to crontab, for example: 0 */6 * * * cd /path/to/idawhats && ./scripts/backup.sh
BACKUP_DIR=/backups/idawhats ./scripts/backup.sh
```
...
2. **Check logs** - `docker compose logs idawhats --tail=100`
...
- **GitHub Issues**: [github.com/FIXFIBER/IdaWhats/issues](https://github.com/FIXFIBER/IdaWhats/issues)
- **Discussions**: [github.com/FIXFIBER/IdaWhats/discussions](https://github.com/FIXFIBER/IdaWhats/discussions)
- **Discord**: [discord.gg/idawhats](https://discord.gg/idawhats) (if available)
- **Stack Overflow**: Tag with `idawhats`
---
