...
IdaWhats provides official n8n community nodes for integrating WhatsApp automation into n8n workflows. This enables users to build powerful automations combining WhatsApp messaging with hundreds of other services available in n8n.

**Repository:** https://github.com/FIXFIBER/IdaWhats-n8n
**npm Package:** `@FIXFIBER/n8n-nodes-idawhats`

## Architecture

```
┌─────────────────┐     ┌─────────────────┐     ┌─────────────────┐
│   n8n Workflow  │────▶│  IdaWhats Node    │────▶│  IdaWhats API     │
│                 │     │  (credentials)  │     │  (your server)  │
└─────────────────┘     └─────────────────┘     └─────────────────┘
                                                        │
                                                        ▼
┌─────────────────┐     ┌─────────────────┐     ┌─────────────────┐
│   n8n Workflow  │◀────│ IdaWhats Trigger  │◀────│  Webhook POST   │
│   (triggered)   │     │  (listens)      │     │  from IdaWhats    │
└─────────────────┘     └─────────────────┘     └─────────────────┘
```
...
2. Select **Install**
3. Enter `@FIXFIBER/n8n-nodes-idawhats`
...
```bash
cd ~/.n8n/nodes
npm install @FIXFIBER/n8n-nodes-idawhats
```
...
Execute operations on your IdaWhats server.

#### Credentials Setup

| Field      | Description                      | Example                  |
| ---------- | -------------------------------- | ------------------------ |
| Server URL | IdaWhats server URL (without /api) | `https://wa.example.com` |
| API Key    | API key from IdaWhats dashboard    | `owa_xxxxxxxx...`        |
...
1. When workflow is activated, the trigger creates a webhook in IdaWhats
2. IdaWhats sends events to n8n's webhook URL
...
> `X-IdaWhats-Idempotency-Key` / `X-IdaWhats-Delivery-Id` headers. `idempotencyKey` is **stable across retries**
...
```
[IdaWhats Trigger] → [IF: Check keyword] → [IdaWhats: Send Text]
     │
...
- IdaWhats: Send Text with welcome message
...
```
[IdaWhats Trigger] → [Google Sheets: Append] → [IdaWhats: Send Text]
     │                    │
...
```
[IdaWhats Trigger] → [Slack: Send Message]
     │
...
```
[Webhook: New Order] → [IdaWhats: Send Text]
                            │
...
```
[Schedule Trigger] → [Google Sheets: Get Rows] → [Loop] → [IdaWhats: Send Text]
     │                      │                                    │
...
```
[IdaWhats Trigger] → [IF: Booking intent?] → [Set: Normalize request]
                                               │
                                               ▼
                                      [Availability Source]
                                               │
                         ┌─────────────────────┴─────────────────────┐
                         ▼                                           ▼
              [Create Booking] → [IdaWhats: Send Text]      [IdaWhats: Send Text]
...
```
[IdaWhats Node] → [IF: Check success] → [Continue...]
                      │
                      └── [Error Handler]
```
...
```
[Loop Over Items] → [Wait: 2 seconds] → [IdaWhats: Send Text]
```
...
1. Verify IdaWhats server is running
2. Check API key is correct
3. Ensure server URL doesn't have trailing slash
4. Verify network connectivity between n8n and IdaWhats

### Trigger Not Receiving Events

1. Check webhook was created in IdaWhats dashboard
2. Verify n8n webhook URL is accessible from IdaWhats server
...
```bash
git clone https://github.com/FIXFIBER/IdaWhats-n8n.git
cd IdaWhats-n8n
npm install
npm run build
```
...
- [IdaWhats API Specification](./06-api-specification.md)
